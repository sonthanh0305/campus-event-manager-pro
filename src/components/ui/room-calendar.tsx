...
import { format, addHours, addDays, startOfWeek, parseISO, isWithinInterval, isSameDay } from "date-fns";
...
export const RoomCalendar = ({
  ...
  const isSlotBooked = (roomId: string, day: Date, timeStr: string) => {
    ...
    return bookings.find(booking => 
      ...
      isWithinInterval(slotStart, { start: booking.start, end: booking.end }) ||
  ...
  const isSlotSelected = (roomId: string, day: Date, timeStr: string) => {
    ...
    return selectedSlots.some(slot => {
      ...
      return slot.id.includes(roomId) && 
    ...
    <div className="border rounded-md">
      <div className="flex items-center justify-between p-4 border-b">
        ...
        <div className="font-semibold">
          {format(days[0], "dd/MM/yyyy", { locale: vi })} - {format(days[6], "dd/MM/yyyy", { locale: vi })}
        </div>
      </div>
          ...
          <div className="grid grid-cols-7 flex-1">
            {days.map((day, index) => (
              ...
              >
                ...
                <div className="text-sm text-muted-foreground">{format(day, 'dd/MM', { locale: vi })}</div>
              </div>
            ))}
          </div>
        </div>
                ...
                {days.map((day, dayIndex) => (
                  <div key={dayIndex} className="border-r">
                    {timeSlots.map((timeSlot, timeIndex) => {
                      ...
                      const booking = isBooked ? getBookingForSlot(room.id, day, timeSlot) : null;
                      const isSelected = isSlotSelected(room.id, day, timeSlot);
                      
                      return (
                        <TooltipProvider key={`${room.id}-${dayIndex}-${timeIndex}`}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div 
                                className={cn(
                                  "border-b h-12 relative cursor-pointer hover:bg-muted/50",
                                  isBooked && booking ? getStatusColor(booking.status) : "",
                                  isSelected && "bg-primary/20 dark:bg-primary/30",
                                  (timeIndex % 2 === 0) && "border-dashed"
                                )}
                                onClick={() => handleTimeSlotClick(room.id, day, timeSlot)}
                              >
                            ...
                            <TooltipContent side="right">
                              {booking ? (
                                <div className="text-xs">
                                  ...
                                  <div>Trạng thái: {booking.status === 'approved' ? 'Đã duyệt' : 
                                    booking.status === 'pending' ? 'Chờ duyệt' : 'Đã từ chối'}</div>
                                </div>
                              ) : (
                                <div className="text-xs">{timeSlot} - Trống</div>
                              )}
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
